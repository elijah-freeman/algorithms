...
public class Sorting {
    ...
    public LinkedNode<Integer> bubbleSort(LinkedNode<Integer> first) {
            ...
            swapCheck = false;
                ...
                if (!callFromShell) {
                    if (exchange != 0 && numberPrints < 20) {
                        writeOutput(first, 0, numPass, comparison, exchange, bubbleFlag, sortType);
                        numberPrints++;
                    }
                }
            }
        ...
        if (!callFromShell) {
            ...
            writeOutput(first, 0, totalPasses, totalComparisons, totalExchanges, bubbleFlag, sortType);
        }
    ...
    public LinkedNode<Integer> shellSort(LinkedNode<Integer> first) {
        ...
        output.println("------------------------------------------");
        output.println("Total:    pass          cmp          exch");
        writeOutput(first, gap, totalPasses, totalComparisons, totalExchanges, shellFlag, sortType);
    ...
    private void writeOutput(LinkedNode<Integer> head, int gap, int thePass, int theComparison, int theExchange, boolean flag, boolean sortType) {

        if (sortType && flag) {

            output.printf("          %-10d%-15d%-15d", thePass, theComparison, theExchange);
            LinkedNode<Integer> current = head;
            //prints List for each pass that is recorded (written in the txt file). Uncomment to view
//            while (current != null) {
